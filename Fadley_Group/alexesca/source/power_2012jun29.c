...
#define DIOBOARDNUMBER	0   //AWK as configured in the Instacal software; we should put this into ini file tag/value
...
static unsigned short port3data, port4data, port5data;
static unsigned short port5Tdata;  //port 5 with command toggle
static double ReferenceEnergy =   1384.035; // value used in origianl tests 1365.7;  	// reference energy for the HP ESCA supply  1372.6??    //AWK: should also be in an ini file tag/value   
static int power_communication_initalized=0;
...
int power_init_communication( int force )
{
	int err=0;
	int error;
	int port=0;
	char errorString[ERRSTRLEN+1];
	static short power_communication_initalized_flag;

	// library port config functino sets ALL ports to zero on any one port config.
	//  ... so we will initalize only first time called or on "force" - AWK 2012jun29

	if( power_communication_initalized_flag == 0 || force>0 ) {
		...
		port= SECONDPORTCL;  // port C is only addressable as HI and LO (4 bits each) for some goofy reason      
		error= cbDConfigPort (DIOBOARDNUMBER, port, DIGITALOUT);
		cbGetErrMsg(error,errorString );
		printf( "cbDConfigPort ( ) error= %i = %s... port 2C-LO\n", error, errorString );       

		port= SECONDPORTCH;  // port C is only addressable as HI and LO (4 bits each) for some goofy reason      
		error= cbDConfigPort (DIOBOARDNUMBER, port, DIGITALOUT);
		cbGetErrMsg(error,errorString );
		printf( "cbDConfigPort ( ) error= %i = %s... port 2C-HI\n", error, errorString );       
		
		power_communication_initalized_flag=1;
	}
	return err;
}
...
int power_send_commands( void )
{
	...
	else 
	{
		...
		//this is all we needed to do with the old NIDAQ library...
		...
		if(method==1){
			...
			board=DIOBOARDNUMBER;
			port= SECONDPORTA;	
			portdata=port3data ;
			error= cbDOut(board, port, portdata);
			cbGetErrMsg(error,errorString );
			printf( "cbDOut (%d, %d, 0x%x) returned %i = %s.\n", board, port, portdata, error, errorString  ); 
			...
			port= SECONDPORTCH;  // port C is only addressable as HI and LO (4 bits) for some goofy reason     
			portdata=port5data;
			error= cbDOut(board, port, portdata >> 4);  //need bit shifts due to 4bit port    
			cbGetErrMsg(error,errorString );    
			printf( "cbDOut (%d, %d, 0x%x) returned %i = %s.\n", board, port, portdata, error, errorString  ); 
		
			//NEED TO TOGGLE GATE BIT TO FORCE INTERFACE TO ACKNOWLEDGE COMMAND
			Delay (0.01);									    
			error= cbDOut(board, port, ( port5data ^ toggle) >>4);   //need bit shifts due to 4bit port
			cbGetErrMsg(error,errorString );
			printf( "cbDOut (%d, %d, 0x%x) returned %i = %s.\n", board, port, portdata, error, errorString  ); 
		
			//COMPLETE TOGGLE
			Delay (0.01);  
			error= cbDOut(board, port, port5data >> 4);//;	//need bit shifts due to 4bit port    
			cbGetErrMsg(error,errorString );   
			printf( "cbDOut (%d, %d, 0x%x) returned %i = %s.\n", board, port, portdata, error, errorString  ); 
		}
		else if (method==2){
			...
			board=DIOBOARDNUMBER;
			port=4;  // cbOutByte fcn actually wants REGISTER number but calls is "port".  Registers are 4,5,6 per their documentation
			portdata= (int)port3data;
			error= cbOutByte (board, port, portdata); 
			cbGetErrMsg(error,errorString ); 
			printf( "cbOutByte (%d, %d, 0x%x) returned %i = %s.\n", board, port, portdata, error, errorString  ); 
		     
			port=5;
			portdata= (int)port4data;
			error= cbOutByte (board, port, portdata); 
			cbGetErrMsg(error,errorString );  
			printf( "cbOutByte (%d, %d, 0x%x) returned %i = %s.\n", board, port, portdata, error, errorString  ); 
		
			port=6;
			portdata= (int)port5data;
			error= cbOutByte (board, port, portdata); 
			cbGetErrMsg(error,errorString ); 
			printf( "cbOutByte (%d, %d, 0x%x) returned %i = %s.\n", board, port, portdata, error, errorString  ); 
		
			Delay (0.01);   
			error= cbOutByte (board, port, portdata ^ toggle); 
			cbGetErrMsg(error,errorString ); 
			printf( "cbOutByte (%d, %d, 0x%x) returned %i = %s.\n", board, port, portdata, error, errorString  );   
