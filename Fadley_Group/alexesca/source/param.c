...
#include "alexmisc.h"	// need function prototypes  


int ParametersPanelOpen=0;
int RegionPanelOpen=0;

//panel handles
static int pParameters, pRegion, pWarning, mRegion,mRgnSelect;   
static int *RegionMenuItem;       // for array of handles for programatically built part of region menu bar

static int n_o_available_regions, n_o_selected_regions;
static struct regionStruct *AvailableRgnArray;      /* will dynamically allocate n_o_available_regions worth of Region later with this */
static struct regionStruct *SelectedRgnArray;    
static struct regionStruct *TempRgnArray;

static struct experimentStruct Expt;								// could make this a pointer to array as above to construct an experiemtn queue!
...
int region_check_if_description_is_already_used( char *Description )
{
	...
	//strip extraneous white space before/after text in description
	strip_white_ends_from_string( Description );
...
struct regionStruct Adjust_region_parameters(  struct regionStruct rgn )
{
	...
	//strip extraneous white space before/after text in description
	strip_white_ends_from_string( rgn.Description );
...
//	printf( " num_reg = %i \n ",  n_o_available_regions );
	...
	{ 
		...
		error = ini_find_section( file, string , '[', ']', ';' , &filePos );
			...
			//strip extraneous white space before/after text in description
			strip_white_ends_from_string( AvailableRgnArray[i].Description );					
...
//		avail_build_list_box ( );	// draws the vertical separation bars...
		mRgnSelect = LoadMenuBar (0, "param.uir", mRGNSELECT);
		...
		n_o_selected_regions=0;
//		for( i = 0 ; i < n_o_selected_regions ; i++ ){
//			 InsertListItem (pParameters, pPARAM_SELECTED_REGIONS, -1, SelectedRgnArray[i].Description, i);
//		}
		
		sprintf( Message, "AVAILABLE REGIONS (%i)", n_o_available_regions );   
		SetCtrlVal (pParameters, pPARAM_N_AVAIL, Message);   
		sprintf( Message, "SELECTED REGIONS (%i)", n_o_selected_regions );
		SetCtrlVal (pParameters, pPARAM_N_SELECTED, Message);   		
		...
		case EVENT_RIGHT_CLICK:
			{
				...
				GetCtrlAttribute (panel, pPARAM_AVAILABLE_REGIONS, ATTR_HEIGHT, &h );
				GetCtrlAttribute (panel, pPARAM_AVAILABLE_REGIONS, ATTR_WIDTH, &w ); 
				GetCtrlAttribute (panel, pPARAM_AVAILABLE_REGIONS, ATTR_LEFT, &l ); 
				GetCtrlAttribute (panel, pPARAM_AVAILABLE_REGIONS, ATTR_TOP, &t ); 
				
				
				menuItem=RunPopupMenu (mRgnSelect, mRGNSELECT_MENU, panel,
												eventData1, eventData2, t, l, h, w);
												
				switch( menuItem )
				{
					case mRGNSELECT_MENU_CUT:
						// ask user if they really want to delete the checked regions...
					
						// count the number of checked regions
						GetNumCheckedItems (panel, pPARAM_AVAILABLE_REGIONS, &n_o_selected_regions );
						if( n_o_selected_regions < 1 ) return 0;
					...
					case mRGNSELECT_MENU_SELECT:
					
						if( n_o_selected_regions < 1 ) break;
						// clear the "selected" list 
						ClearListCtrl (panel, pPARAM_SELECTED_REGIONS);
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event) {
		case EVENT_COMMIT:
			switch ( control ) 
			{
				case pPARAM_DESCRIPTION:
					GetCtrlVal (pParameters ,pPARAM_DESCRIPTION, 	Expt.Description	);
					strip_white_ends_from_string( Expt.Description );
					SetCtrlVal (pParameters ,pPARAM_DESCRIPTION, 	Expt.Description	);               
					break;
...
int CVICALLBACK param_layers_control (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_COMMIT:
			...
			switch( control )
			{
				case pPARAM_LAYERS_NONE:
					SetCtrlVal (panel, pPARAM_LAYERS_NONE, 1);
					SetCtrlVal (panel, pPARAM_LAYERS_MANUAL, 0);
					SetCtrlVal (panel, pPARAM_LAYERS_POLAR, 0);
					SetCtrlVal (panel, pPARAM_LAYERS_TIMED, 0); 
					
					 
					SetCtrlAttribute (panel, pPARAM_LAYERS_MANUAL_NSTEPS,ATTR_VISIBLE,	0);
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_NSTEPS, ATTR_VISIBLE, 	0);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_INIT, ATTR_VISIBLE, 	0);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_FINAL,ATTR_VISIBLE,	0);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_STEP, ATTR_VISIBLE, 	0);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_TIMED_NSTEPS, ATTR_VISIBLE, 	0);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_TIMED_TIME, ATTR_VISIBLE, 	0);    
					
					SetCtrlVal (panel, pPARAM_LAYERS_MANUAL_NSTEPS, 1);
					
					sprintf( Expt.Layers.Description, "XPS" );
					Expt.Layers.Type = LAYER_TYPE_NONE;
					
					update_layer_display();          
					break;
					
				case pPARAM_LAYERS_MANUAL:
					SetCtrlVal (panel, pPARAM_LAYERS_NONE, 0);
					SetCtrlVal (panel, pPARAM_LAYERS_MANUAL,1);
					SetCtrlVal (panel, pPARAM_LAYERS_POLAR, 0);
					SetCtrlVal (panel, pPARAM_LAYERS_TIMED, 0); 
					
					SetCtrlAttribute (panel, pPARAM_LAYERS_MANUAL_NSTEPS,ATTR_VISIBLE,	1);
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_NSTEPS, ATTR_VISIBLE, 	0);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_INIT, ATTR_VISIBLE, 	0);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_FINAL,ATTR_VISIBLE,	0);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_STEP, ATTR_VISIBLE, 	0);   
					...
					Expt.Layers.Type = LAYER_TYPE_MANUAL;
					
					update_layer_display();          
					break;
					
				case pPARAM_LAYERS_POLAR:
					SetCtrlVal (panel, pPARAM_LAYERS_NONE, 0);
					SetCtrlVal (panel, pPARAM_LAYERS_MANUAL,0);
					SetCtrlVal (panel, pPARAM_LAYERS_POLAR, 1);
					SetCtrlVal (panel, pPARAM_LAYERS_TIMED, 0); 
					
					SetCtrlAttribute (panel, pPARAM_LAYERS_MANUAL_NSTEPS,ATTR_VISIBLE,	0);
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_NSTEPS, ATTR_VISIBLE, 	1);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_INIT, ATTR_VISIBLE, 	1);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_FINAL,ATTR_VISIBLE,	1);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_STEP, ATTR_VISIBLE, 	1);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_TIMED_NSTEPS, ATTR_VISIBLE, 	0);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_TIMED_TIME, ATTR_VISIBLE, 	0);   
					
					SetCtrlAttribute (panel, pPARAM_LAYERS_MANUAL_NSTEPS, ATTR_VISIBLE, 0); 
					
					Expt.Layers.Type = LAYER_TYPE_POLAR;
					
					update_layer_display();          
					break;
					
				case pPARAM_LAYERS_TIMED:
					SetCtrlVal (panel, pPARAM_LAYERS_NONE, 0);
					SetCtrlVal (panel, pPARAM_LAYERS_MANUAL,0);
					SetCtrlVal (panel, pPARAM_LAYERS_POLAR, 0);
					SetCtrlVal (panel, pPARAM_LAYERS_TIMED, 1); 
					...
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_FINAL,ATTR_VISIBLE,	0);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_POLAR_STEP, ATTR_VISIBLE, 	0);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_TIMED_NSTEPS, ATTR_VISIBLE, 	1);   
					SetCtrlAttribute (panel, pPARAM_LAYERS_TIMED_TIME, ATTR_VISIBLE, 	1);   
					
					SetCtrlAttribute (panel, pPARAM_LAYERS_MANUAL_NSTEPS, ATTR_VISIBLE, 0); 
					
					Expt.Layers.Type = LAYER_TYPE_TIMED;
					
					update_layer_display();          
					break;
				
				case pPARAM_LAYERS_MANUAL_NSTEPS:
					GetCtrlVal( panel, control,  &Expt.Layers.Manual.Size );
					Expt.Layers.Manual.Size++;
					
					update_layer_display();          
					break;
				
				//============================================
				case pPARAM_LAYERS_POLAR_NSTEPS: 
					GetCtrlVal( panel, control, &Expt.Layers.Polar.Size );
					Expt.Layers.Polar.Size++;
					
					//validate values
					if( Expt.Layers.Polar.Initial < -360.0 ) Expt.Layers.Polar.Initial = -360.0; 
					else if( Expt.Layers.Polar.Initial > +360.0 ) Expt.Layers.Polar.Initial = +360.0;
					
					if( Expt.Layers.Polar.Final < -360.0 ) Expt.Layers.Polar.Final  = -360.0; 
					else if( Expt.Layers.Polar.Final  > +360.0 ) Expt.Layers.Polar.Final  = +360.0;
					
					if( Expt.Layers.Polar.Size < 1 ) Expt.Layers.Polar.Size = 1; 
					if( Expt.Layers.Polar.Final != Expt.Layers.Polar.Initial  && Expt.Layers.Polar.Size < 2 )
						Expt.Layers.Polar.Size = 2;
					if( Expt.Layers.Polar.Final == Expt.Layers.Polar.Initial  && Expt.Layers.Polar.Size != 1)
						Expt.Layers.Polar.Size = 1;
						
						
					//accept initial, size, step and adjust final to match..
					// if final is beyond range, decrease the size until ok
					Expt.Layers.Polar.Final = Expt.Layers.Polar.Initial +
															Expt.Layers.Polar.Step*(double)Expt.Layers.Polar.Size; 
					while( Expt.Layers.Polar.Final > 360.0)
					{
						Expt.Layers.Polar.Size--;
						Expt.Layers.Polar.Final = Expt.Layers.Polar.Initial +
															Expt.Layers.Polar.Step*(double)Expt.Layers.Polar.Size; 
				...
				case pPARAM_LAYERS_POLAR_INIT:
					GetCtrlVal( panel, control, &Expt.Layers.Polar.Initial ); 
					...
					if( Expt.Layers.Polar.Size > 1 )
						Expt.Layers.Polar.Step =  
									(Expt.Layers.Polar.Final - Expt.Layers.Polar.Initial) /
											((double) Expt.Layers.Polar.Size-1);
					else  Expt.Layers.Polar.Step = (Expt.Layers.Polar.Final - Expt.Layers.Polar.Initial);    
					
					update_layer_display();          
					break;
					
				case pPARAM_LAYERS_POLAR_FINAL:
					GetCtrlVal( panel, control, &Expt.Layers.Polar.Final );
					...
					//accept initial, final, size and adjust step to match
					if( Expt.Layers.Polar.Size > 1 )  
						Expt.Layers.Polar.Step =  
							(Expt.Layers.Polar.Final - Expt.Layers.Polar.Initial) / 
										((double) Expt.Layers.Polar.Size-1);
				...
				case pPARAM_LAYERS_POLAR_STEP:  //step size
					GetCtrlVal( panel, control, &Expt.Layers.Polar.Step ); 
				...
				case pPARAM_LAYERS_TIMED_NSTEPS:
					GetCtrlVal( panel, control, &Expt.Layers.Timed.Size );
					Expt.Layers.Timed.Size++;
					
					//validate
					if( Expt.Layers.Timed.Size < 1 ) Expt.Layers.Timed.Size = 1;    
					if( Expt.Layers.Timed.Time <0.0 ) Expt.Layers.Timed.Size = 0.0;
					
					update_layer_display();          
					break;
					
				case pPARAM_LAYERS_TIMED_TIME:
					GetCtrlVal( panel, control, &Expt.Layers.Timed.Time );
					//validate
					if( Expt.Layers.Timed.Size < 1 ) Expt.Layers.Timed.Size = 1;    
					if( Expt.Layers.Timed.Time <0.0 ) Expt.Layers.Timed.Size = 0.0;
...
void layers_set_default(void)
{
	...
	sprintf( Expt.Layers.Description, "None" );
	Expt.Layers.Type = LAYER_TYPE_NONE;
	
	Expt.Layers.Manual.Size = 1;
	
	Expt.Layers.Polar.Size = 1;
	Expt.Layers.Polar.Initial = 45.0;
	Expt.Layers.Polar.Final = Expt.Layers.Polar.Initial;
	
	Expt.Layers.Timed.Size =1;
	Expt.Layers.Timed.Time =60.0;	//60 sec = 1 minute
	
	SetCtrlVal (pParameters, pPARAM_LAYERS_NONE, 	1);
	SetCtrlVal (pParameters, pPARAM_LAYERS_MANUAL, 	0);
	SetCtrlVal (pParameters, pPARAM_LAYERS_POLAR, 	0);      
	SetCtrlVal (pParameters, pPARAM_LAYERS_TIMED, 	0);
	
	
	update_layer_display();
	
	
	SetCtrlAttribute (pParameters, pPARAM_LAYERS_MANUAL_NSTEPS,	ATTR_VISIBLE,	0);
	SetCtrlAttribute (pParameters, pPARAM_LAYERS_POLAR_NSTEPS, 	ATTR_VISIBLE, 	0);   
	SetCtrlAttribute (pParameters, pPARAM_LAYERS_POLAR_INIT, 	ATTR_VISIBLE, 	0);   
	SetCtrlAttribute (pParameters, pPARAM_LAYERS_POLAR_FINAL,	ATTR_VISIBLE,	0);   
	SetCtrlAttribute (pParameters, pPARAM_LAYERS_POLAR_STEP, 	ATTR_VISIBLE, 	0);   
	SetCtrlAttribute (pParameters, pPARAM_LAYERS_TIMED_NSTEPS, 	ATTR_VISIBLE, 	0);   
	SetCtrlAttribute (pParameters, pPARAM_LAYERS_TIMED_TIME, 	ATTR_VISIBLE, 	0);
	
	SetCtrlVal (pParameters, pPARAM_LAYERS_MANUAL_NSTEPS, 1);
}


void update_layer_display( void)
{
	SetCtrlVal (pParameters, pPARAM_LAYERS_MANUAL_NSTEPS,	Expt.Layers.Manual.Size-1	);
	
	SetCtrlVal (pParameters, pPARAM_LAYERS_POLAR_NSTEPS, 	Expt.Layers.Polar.Size-1	);   
	SetCtrlVal (pParameters, pPARAM_LAYERS_POLAR_INIT,  	Expt.Layers.Polar.Initial	);   
	SetCtrlVal (pParameters, pPARAM_LAYERS_POLAR_FINAL,	Expt.Layers.Polar.Final		);
	SetCtrlVal (pParameters, pPARAM_LAYERS_POLAR_STEP,  	Expt.Layers.Polar.Step 		);	
	
	SetCtrlVal (pParameters, pPARAM_LAYERS_TIMED_NSTEPS,  	Expt.Layers.Timed.Size-1	);   
	SetCtrlVal (pParameters, pPARAM_LAYERS_TIMED_TIME, 	Expt.Layers.Timed.Time		);
