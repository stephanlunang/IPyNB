...
{ 
	int i;
	unsigned short value; //awk2012jul05
	...
	else
	{
		...
		//   when the buffers are swapped"...p.6 of Model 2412A Histogramming Buffer Memory Manual
			...
			//awk2012jul05
			value =  (unsigned short) inpw ( BASE_2420B );	// read the data that is present   inpw=word=2bytes=16bits
			array[i] = (double)value;

			if( value & 0x8000 ) // bitwise and to check for last bit high
				printf( "WARNING:  Detector Overrun flag!  Channel %d, value=%d\n", i, value);
			
			
			//awk2012jul05 array[N_O_DETECTOR_CHANNELS - 1] = 0.0  ;
			//awk2012jul05 array[N_O_DETECTOR_CHANNELS - 2] = 0.0  ;
		
			//Strobe the next channel data onto the output lines
			det_strobe_IOSTB();
		}
		printf( "Detect readout complete.\n"); 
		
		//awk2012jul05 ; these channels are always junk based on my old comments (just set once rather than on every channel read as above
